EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:switches
LIBS:relays
LIBS:motors
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:teensy
LIBS:XBP9B-DMST-002
LIBS:PLEC
LIBS:cap_470u
LIBS:PLEC-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 9
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 1600 4900 1050 1500
U 5A71515B
F0 "Voltage Dividers" 60
F1 "PLEC_dividers.sch" 60
F2 "IN1" I L 1600 5100 60 
F3 "IN2" I L 1600 5200 60 
F4 "IN3" I L 1600 5300 60 
F5 "IN4" I L 1600 5400 60 
F6 "IN5" I L 1600 5500 60 
F7 "IN6" I L 1600 5600 60 
F8 "IN7" I L 1600 5700 60 
F9 "IN8" I L 1600 5800 60 
F10 "IN9" I L 1600 5900 60 
F11 "IN0" I L 1600 5000 60 
F12 "OUT0" O R 2650 5000 60 
F13 "OUT1" O R 2650 5100 60 
F14 "OUT2" O R 2650 5200 60 
F15 "OUT3" O R 2650 5300 60 
F16 "OUT4" O R 2650 5400 60 
F17 "OUT5" O R 2650 5500 60 
F18 "OUT6" O R 2650 5600 60 
F19 "OUT7" O R 2650 5700 60 
F20 "OUT8" O R 2650 5800 60 
F21 "OUT9" O R 2650 5900 60 
F22 "VEE" O L 1600 6300 60 
F23 "INA" I L 1600 6050 60 
F24 "OUTA" O R 2650 6050 60 
F25 "INB" I L 1600 6150 60 
F26 "OUTB" O R 2650 6150 60 
$EndSheet
$Sheet
S 1650 1000 1000 500 
U 5A71AC48
F0 "Power Rails" 60
F1 "PLEC_power.sch" 60
F2 "VCC_9" I L 1650 1100 60 
F3 "VCC_5" O R 2650 1250 60 
F4 "VEE" O L 1650 1200 60 
F5 "VCC_9_OL" O R 2650 1100 60 
F6 "VCC_33" O R 2650 1400 60 
$EndSheet
$Comp
L Conn_01x02 J1
U 1 1 5A72CED8
P 950 1100
F 0 "J1" H 950 1200 50  0000 C CNN
F 1 "Conn_01x02" H 950 900 50  0000 C CNN
F 2 "PLEC:THT_TerminalBlock_2.5mm_2pos" H 950 1100 50  0001 C CNN
F 3 "" H 950 1100 50  0001 C CNN
	1    950  1100
	-1   0    0    -1  
$EndComp
$Comp
L Conn_01x03 J3
U 1 1 5A72D02D
P 10900 900
F 0 "J3" H 10900 1100 50  0000 C CNN
F 1 "Conn_01x03" H 10900 700 50  0000 C CNN
F 2 "PLEC:THT_TerminalBlock_2.5mm_3pos" H 10900 900 50  0001 C CNN
F 3 "" H 10900 900 50  0001 C CNN
	1    10900 900 
	1    0    0    -1  
$EndComp
$Comp
L Conn_01x04 J5
U 1 1 5A72D162
P 10900 4000
F 0 "J5" H 10900 4200 50  0000 C CNN
F 1 "Conn_01x04" H 10900 3700 50  0000 C CNN
F 2 "PLEC:THT_TerminalBlock_2.5mm_4pos" H 10900 4000 50  0001 C CNN
F 3 "" H 10900 4000 50  0001 C CNN
	1    10900 4000
	1    0    0    -1  
$EndComp
$Comp
L Conn_01x06 J4
U 1 1 5A72D6B1
P 10900 3150
F 0 "J4" H 10900 3450 50  0000 C CNN
F 1 "Conn_01x06" H 10900 2750 50  0000 C CNN
F 2 "PLEC:THT_TerminalBlock_2.5mm_6pos" H 10900 3150 50  0001 C CNN
F 3 "" H 10900 3150 50  0001 C CNN
	1    10900 3150
	1    0    0    -1  
$EndComp
Text GLabel 2650 1250 2    60   Output ~ 0
V_5
Text GLabel 2650 1400 2    60   Output ~ 0
V_33
Text GLabel 1100 2200 0    60   Input ~ 0
V_33
Text GLabel 2650 1100 2    60   Output ~ 0
V_9
Text GLabel 10700 800  0    60   Input ~ 0
V_9
Text GLabel 10700 2950 0    60   Input ~ 0
EM_R1+
Text GLabel 10700 3050 0    60   Input ~ 0
EM_R1-
Text GLabel 10700 3150 0    60   Input ~ 0
EM_R2+
Text GLabel 10700 3250 0    60   Input ~ 0
EM_R2-
Text GLabel 10700 3350 0    60   Input ~ 0
EM_R3+
Text GLabel 10700 3450 0    60   Input ~ 0
EM_R3-
Text GLabel 10700 3900 0    60   Input ~ 0
EM_E1+
Text GLabel 10700 4000 0    60   Input ~ 0
EM_E1-
Text GLabel 10700 4100 0    60   Input ~ 0
EM_E2+
Text GLabel 10700 4200 0    60   Input ~ 0
EM_E2-
Text GLabel 1600 5000 0    60   Input ~ 0
EM_R1+
Text GLabel 1600 5100 0    60   Input ~ 0
EM_R1-
Text GLabel 1600 5200 0    60   Input ~ 0
EM_R2+
Text GLabel 1600 5300 0    60   Input ~ 0
EM_R2-
Text GLabel 1600 5400 0    60   Input ~ 0
EM_R3+
Text GLabel 1600 5500 0    60   Input ~ 0
EM_R3-
Text GLabel 1600 5600 0    60   Input ~ 0
EM_E1+
Text GLabel 1600 5700 0    60   Input ~ 0
EM_E1-
Text GLabel 1600 5800 0    60   Input ~ 0
EM_E2+
Text GLabel 1600 5900 0    60   Input ~ 0
EM_E2-
Text GLabel 2650 5000 2    60   Output ~ 0
EM_R1_VD+
Text GLabel 2650 5100 2    60   Output ~ 0
EM_R1_VD-
Text GLabel 2650 5200 2    60   Output ~ 0
EM_R2_VD+
Text GLabel 2650 5300 2    60   Output ~ 0
EM_R2_VD-
Text GLabel 2650 5400 2    60   Output ~ 0
EM_R3_VD+
Text GLabel 2650 5500 2    60   Output ~ 0
EM_R3_VD-
Text GLabel 2650 5600 2    60   Output ~ 0
EM_E1_VD+
Text GLabel 2650 5700 2    60   Output ~ 0
EM_E1_VD-
Text GLabel 2650 5800 2    60   Output ~ 0
EM_E2_VD+
Text GLabel 2650 5900 2    60   Output ~ 0
EM_E2_VD-
Text GLabel 2500 2200 2    60   Output ~ 0
UART_TIXO
Text GLabel 2500 2400 2    60   Output ~ 0
UART_~CTS
Text GLabel 2500 2700 2    60   Output ~ 0
XB_~INTERRUPT
Text GLabel 2500 3000 2    60   Output ~ 0
SPI_MISO
Text GLabel 2600 3700 2    60   Input ~ 0
XB_~SLP
Text GLabel 2600 3550 2    60   Input ~ 0
XB_~RST
Text GLabel 2500 3200 2    60   Input ~ 0
SPI_SS
Text GLabel 2500 3100 2    60   Input ~ 0
SPI_MOSI
Text GLabel 2500 2900 2    60   Input ~ 0
SPI_CLK
Text GLabel 2500 2500 2    60   Input ~ 0
UART_~RTS
Text GLabel 2500 2300 2    60   Input ~ 0
UART_TOXI
$Sheet
S 8550 2850 1050 1450
U 5A746317
F0 "Relays" 60
F1 "PLEC_relays.sch" 60
F2 "VDD" I L 8550 2950 60 
F3 "R1D_IN" I L 8550 3100 60 
F4 "R2D_IN" I L 8550 3200 60 
F5 "R3D_IN" I L 8550 3300 60 
F6 "R4D_IN" I L 8550 3400 60 
F7 "R5D_IN" I L 8550 3500 60 
F8 "R12S_IN" I L 8550 3700 60 
F9 "R3S_IN" I L 8550 3800 60 
F10 "R45S_IN" I L 8550 3900 60 
F11 "R1D_OUT" O R 9600 2950 60 
F12 "R1S_OUT" O R 9600 3050 60 
F13 "R2D_OUT" O R 9600 3200 60 
F14 "R2S_OUT" O R 9600 3300 60 
F15 "R3D_OUT" O R 9600 3450 60 
F16 "R3S_OUT" O R 9600 3550 60 
F17 "R4D_OUT" O R 9600 3700 60 
F18 "R4S_OUT" O R 9600 3800 60 
F19 "R5D_OUT" O R 9600 3950 60 
F20 "R5S_OUT" O R 9600 4050 60 
F21 "VSS" O R 9600 4200 60 
F22 "GND_SENSED" O L 8550 4200 60 
$EndSheet
Text GLabel 9600 2950 2    60   Output ~ 0
EM_R1+
Text GLabel 9600 3050 2    60   Output ~ 0
EM_R1-
Text GLabel 9600 3200 2    60   Output ~ 0
EM_R2+
Text GLabel 9600 3300 2    60   Output ~ 0
EM_R2-
Text GLabel 9600 3450 2    60   Output ~ 0
EM_R3+
Text GLabel 9600 3550 2    60   Output ~ 0
EM_R3-
Text GLabel 9600 3700 2    60   Output ~ 0
EM_E1+
Text GLabel 9600 3800 2    60   Output ~ 0
EM_E1-
Text GLabel 9600 3950 2    60   Output ~ 0
EM_E2+
Text GLabel 9600 4050 2    60   Output ~ 0
EM_E2-
$Comp
L GND #PWR01
U 1 1 5A755B8B
P 1000 3200
F 0 "#PWR01" H 1000 2950 50  0001 C CNN
F 1 "GND" H 1000 3050 50  0000 C CNN
F 2 "" H 1000 3200 50  0001 C CNN
F 3 "" H 1000 3200 50  0001 C CNN
	1    1000 3200
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR02
U 1 1 5A757A08
P 1500 6300
F 0 "#PWR02" H 1500 6050 50  0001 C CNN
F 1 "GND" H 1500 6150 50  0000 C CNN
F 2 "" H 1500 6300 50  0001 C CNN
F 3 "" H 1500 6300 50  0001 C CNN
	1    1500 6300
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR03
U 1 1 5A75832C
P 1350 1300
F 0 "#PWR03" H 1350 1050 50  0001 C CNN
F 1 "GND" H 1350 1150 50  0000 C CNN
F 2 "" H 1350 1300 50  0001 C CNN
F 3 "" H 1350 1300 50  0001 C CNN
	1    1350 1300
	1    0    0    -1  
$EndComp
$Sheet
S 8550 4600 1300 750 
U 5A75C684
F0 "Current Sensor" 60
F1 "PLEC_currentSensor.sch" 60
F2 "VCC" I L 8550 4700 60 
F3 "Output" O R 9850 4700 60 
F4 "CurrentIn" I L 8550 4900 60 
F5 "VEE" O R 9850 5250 60 
F6 "CurrentOut" O L 8550 5000 60 
F7 "Calibrate" I L 8550 5250 60 
$EndSheet
$Comp
L GND #PWR04
U 1 1 5A75FED0
P 9950 5250
F 0 "#PWR04" H 9950 5000 50  0001 C CNN
F 1 "GND" H 9950 5100 50  0000 C CNN
F 2 "" H 9950 5250 50  0001 C CNN
F 3 "" H 9950 5250 50  0001 C CNN
	1    9950 5250
	1    0    0    -1  
$EndComp
Text GLabel 8550 4700 0    60   Input ~ 0
V_5
Text GLabel 9850 4700 2    60   Output ~ 0
CurrentData_5V
Text GLabel 1600 6050 0    60   Input ~ 0
SWITCH_RETURN
Text GLabel 2650 6050 2    60   Output ~ 0
ARMED_3V
Text GLabel 8550 3200 0    60   Input ~ 0
RELAY_2_DRAIN
Text GLabel 8550 3100 0    60   Input ~ 0
RELAY_1_DRAIN
Text GLabel 8550 3300 0    60   Input ~ 0
RELAY_3_DRAIN
Text GLabel 8550 3400 0    60   Input ~ 0
RELAY_4_DRAIN
Text GLabel 8550 3500 0    60   Input ~ 0
RELAY_5_DRAIN
Text GLabel 8550 3700 0    60   Input ~ 0
RLY_12_SHNT_SRC
Text GLabel 8550 3800 0    60   Input ~ 0
RLY_3_SHNT_SRC
Text GLabel 8550 3900 0    60   Input ~ 0
RLY_45_SHNT_SRC
Wire Wire Line
	1150 1200 1650 1200
Wire Wire Line
	1150 1100 1650 1100
Wire Wire Line
	1000 3200 1100 3200
Wire Wire Line
	1500 6300 1600 6300
Connection ~ 1350 1200
Wire Wire Line
	1350 1300 1350 1200
Wire Wire Line
	8550 4200 8250 4200
Wire Wire Line
	9850 5250 9950 5250
Wire Wire Line
	8250 4200 8250 4900
Wire Wire Line
	8250 4900 8550 4900
$Comp
L GND #PWR05
U 1 1 5A744B39
P 8450 5000
F 0 "#PWR05" H 8450 4750 50  0001 C CNN
F 1 "GND" H 8450 4850 50  0000 C CNN
F 2 "" H 8450 5000 50  0001 C CNN
F 3 "" H 8450 5000 50  0001 C CNN
	1    8450 5000
	-1   0    0    -1  
$EndComp
Wire Wire Line
	8550 5000 8450 5000
Text GLabel 1600 6150 0    60   Input ~ 0
CurrentData_5V
Text GLabel 2650 6150 2    60   Output ~ 0
CurrentData_2.5V
Text GLabel 8550 5250 0    60   Input ~ 0
CURR_REF
$Sheet
S 8950 1350 1000 500 
U 5A751D08
F0 "LED Control" 60
F1 "PLEC_blinker.sch" 60
F2 "VEE" O R 9950 1750 60 
F3 "VCC" I L 8950 1450 60 
F4 "Gate" I L 8950 1750 60 
F5 "SwtichedGround" I L 8950 1600 60 
$EndSheet
$Comp
L GND #PWR06
U 1 1 5A757388
P 10050 1750
F 0 "#PWR06" H 10050 1500 50  0001 C CNN
F 1 "GND" H 10050 1600 50  0000 C CNN
F 2 "" H 10050 1750 50  0001 C CNN
F 3 "" H 10050 1750 50  0001 C CNN
	1    10050 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	9950 1750 10050 1750
Text GLabel 8950 1750 0    60   Input ~ 0
LED_CONTROL
$Comp
L Conn_01x02 J2
U 1 1 5A759C02
P 9250 5900
F 0 "J2" H 9250 6000 50  0000 C CNN
F 1 "Conn_01x02" H 9250 5700 50  0000 C CNN
F 2 "PLEC:THT_TerminalBlock_2.5mm_2pos" H 9250 5900 50  0001 C CNN
F 3 "" H 9250 5900 50  0001 C CNN
	1    9250 5900
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR07
U 1 1 5A759FAD
P 8850 6000
F 0 "#PWR07" H 8850 5750 50  0001 C CNN
F 1 "GND" H 8850 5850 50  0000 C CNN
F 2 "" H 8850 6000 50  0001 C CNN
F 3 "" H 8850 6000 50  0001 C CNN
	1    8850 6000
	-1   0    0    -1  
$EndComp
Wire Wire Line
	9050 6000 8850 6000
Text GLabel 9050 5900 0    60   Input ~ 0
ROVER_RELAY
$Sheet
S 1100 2100 1400 1200
U 5A71BFC6
F0 "RF Module" 60
F1 "PLEC_xbee.sch" 60
F2 "VCC_33" I L 1100 2200 60 
F3 "UART_TX" O R 2500 2200 60 
F4 "UART_RX" I R 2500 2300 60 
F5 "SPI_TX" O R 2500 3000 60 
F6 "SPI_RX" I R 2500 3100 60 
F7 "SPI_SS" I R 2500 3200 60 
F8 "SPI_CLK" I R 2500 2900 60 
F9 "VEE" O L 1100 3200 60 
F10 "~RESET" I L 1100 3050 60 
F11 "SLEEP_RQ_~DTR" I L 1100 2900 60 
F12 "~UART_CTS" O R 2500 2400 60 
F13 "~UART_RTS" I R 2500 2500 60 
F14 "~SPI_INT~_~UART_INT" O R 2500 2700 60 
$EndSheet
$Sheet
S 1400 6800 1300 350 
U 5A76BB86
F0 "Pressure Transducer" 60
F1 "PLEC_pressureXdr.sch" 60
F2 "VCC" I L 1400 6900 60 
F3 "VEE" O L 1400 7050 60 
F4 "V_OUT-" O R 2700 7050 60 
F5 "V_OUT+" O R 2700 6900 60 
$EndSheet
Text GLabel 1400 6900 0    60   Input ~ 0
V_5
$Comp
L GND #PWR08
U 1 1 5A76D5B2
P 1300 7050
F 0 "#PWR08" H 1300 6800 50  0001 C CNN
F 1 "GND" H 1300 6900 50  0000 C CNN
F 2 "" H 1300 7050 50  0001 C CNN
F 3 "" H 1300 7050 50  0001 C CNN
	1    1300 7050
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 7050 1400 7050
Text GLabel 2700 6900 2    60   Output ~ 0
PRESSURE+
Text GLabel 2700 7050 2    60   Output ~ 0
PRESSURE-
Wire Wire Line
	8000 2950 8550 2950
Wire Wire Line
	8000 900  8000 2950
Wire Wire Line
	8000 900  10700 900 
Wire Wire Line
	8950 1450 8000 1450
Connection ~ 8000 1450
Text Label 8100 900  0    60   ~ 0
SWITCHED_9V
Wire Wire Line
	10700 1000 8750 1000
Wire Wire Line
	8750 1000 8750 1600
Wire Wire Line
	8750 1600 8950 1600
Text Label 9100 1000 0    60   ~ 0
LED_CATHODE
Text GLabel 10150 2350 2    60   Output ~ 0
SWITCH_RETURN
Wire Wire Line
	10150 2350 8000 2350
Connection ~ 8000 2350
Wire Wire Line
	1100 3050 800  3050
Wire Wire Line
	800  3050 800  3550
Wire Wire Line
	800  3550 2600 3550
Wire Wire Line
	1100 2900 650  2900
Wire Wire Line
	650  2900 650  3700
Wire Wire Line
	650  3700 2600 3700
$Sheet
S 4800 2050 1400 3200
U 5A7803D4
F0 "Microcontroller" 60
F1 "PLEC_teensy36.sch" 60
F2 "UART_RX" I L 4800 2350 60 
F3 "UART_TX" O L 4800 2450 60 
F4 "UART_~CTS" I L 4800 2550 60 
F5 "UART_~RTS" O L 4800 2650 60 
F6 "DATA_~INTERRUPT" I L 4800 2850 60 
F7 "SPI_CLK" O L 4800 3050 60 
F8 "SPI_MISO" I L 4800 3150 60 
F9 "SPI_MOSI" O L 4800 3250 60 
F10 "SPI_SS" O L 4800 3350 60 
F11 "RF_~RESET" O L 4800 3550 60 
F12 "RF_~SLEEP" O L 4800 3650 60 
F13 "A0" I L 4800 3850 60 
F14 "A1" I L 4800 3950 60 
F15 "A2" I L 4800 4050 60 
F16 "A3" I L 4800 4150 60 
F17 "A4" I L 4800 4250 60 
F18 "A5" I L 4800 4350 60 
F19 "A6" I L 4800 4450 60 
F20 "A7" I L 4800 4550 60 
F21 "A8" I L 4800 4650 60 
F22 "A9" I L 4800 4750 60 
F23 "A10" I L 4800 4850 60 
F24 "A11" I L 4800 4950 60 
F25 "A12" I L 4800 5050 60 
F26 "VCC" I L 4800 2150 60 
F27 "A13" I L 4800 5150 60 
F28 "D0" O R 6200 2200 60 
F29 "D1" O R 6200 2400 60 
F30 "D2" O R 6200 2500 60 
F31 "D3" O R 6200 2600 60 
F32 "D4" O R 6200 2700 60 
F33 "D5" O R 6200 2800 60 
F34 "D6" O R 6200 3000 60 
F35 "D7" O R 6200 3100 60 
F36 "D8" O R 6200 3200 60 
F37 "DAC0" O R 6200 3600 60 
F38 "D9" O R 6200 3400 60 
F39 "VEE" O R 6200 5150 60 
$EndSheet
Text GLabel 4800 2350 0    60   Input ~ 0
UART_TIXO
Text GLabel 4800 2550 0    60   Input ~ 0
UART_~CTS
Text GLabel 4800 2850 0    60   Input ~ 0
XB_~INTERRUPT
Text GLabel 4800 3150 0    60   Input ~ 0
SPI_MISO
Text GLabel 4800 3650 0    60   Output ~ 0
XB_~SLP
Text GLabel 4800 3550 0    60   Output ~ 0
XB_~RST
Text GLabel 4800 3350 0    60   Output ~ 0
SPI_SS
Text GLabel 4800 3250 0    60   Output ~ 0
SPI_MOSI
Text GLabel 4800 3050 0    60   Output ~ 0
SPI_CLK
Text GLabel 4800 2650 0    60   Output ~ 0
UART_~RTS
Text GLabel 4800 2450 0    60   Output ~ 0
UART_TOXI
Text GLabel 4800 2150 0    60   Input ~ 0
V_5
Text GLabel 4800 3850 0    60   Input ~ 0
EM_R1_VD+
Text GLabel 4800 3950 0    60   Input ~ 0
EM_R1_VD-
Text GLabel 4800 4050 0    60   Input ~ 0
EM_R2_VD+
Text GLabel 4800 4150 0    60   Input ~ 0
EM_R2_VD-
Text GLabel 4800 4250 0    60   Input ~ 0
EM_R3_VD+
Text GLabel 4800 4350 0    60   Input ~ 0
EM_R3_VD-
Text GLabel 4800 4450 0    60   Input ~ 0
EM_E1_VD+
Text GLabel 4800 4550 0    60   Input ~ 0
EM_E1_VD-
Text GLabel 4800 4650 0    60   Input ~ 0
EM_E2_VD+
Text GLabel 4800 4750 0    60   Input ~ 0
EM_E2_VD-
Text GLabel 4800 4850 0    60   Input ~ 0
ARMED_3V
Text GLabel 4800 4950 0    60   Input ~ 0
CurrentData_2.5V
Text GLabel 4800 5050 0    60   Input ~ 0
PRESSURE+
Text GLabel 4800 5150 0    60   Input ~ 0
PRESSURE-
Text GLabel 6200 2200 2    60   Output ~ 0
LED_CONTROL
Text GLabel 6200 2500 2    60   Output ~ 0
RELAY_2_DRAIN
Text GLabel 6200 2400 2    60   Output ~ 0
RELAY_1_DRAIN
Text GLabel 6200 2600 2    60   Output ~ 0
RELAY_3_DRAIN
Text GLabel 6200 2700 2    60   Output ~ 0
RELAY_4_DRAIN
Text GLabel 6200 2800 2    60   Output ~ 0
RELAY_5_DRAIN
Text GLabel 6200 3000 2    60   Output ~ 0
RLY_12_SHNT_SRC
Text GLabel 6200 3100 2    60   Output ~ 0
RLY_3_SHNT_SRC
Text GLabel 6200 3200 2    60   Output ~ 0
RLY_45_SHNT_SRC
Text GLabel 6200 3600 2    60   Output ~ 0
CURR_REF
Text GLabel 6200 3400 2    60   Output ~ 0
ROVER_RELAY
$Comp
L GND #PWR09
U 1 1 5A76F1F2
P 9700 4200
F 0 "#PWR09" H 9700 3950 50  0001 C CNN
F 1 "GND" H 9700 4050 50  0000 C CNN
F 2 "" H 9700 4200 50  0001 C CNN
F 3 "" H 9700 4200 50  0001 C CNN
	1    9700 4200
	1    0    0    -1  
$EndComp
Wire Wire Line
	9600 4200 9700 4200
$EndSCHEMATC
